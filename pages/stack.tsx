        ...
        <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 pt-16 place-items-center'>
          {stackProps.map((icon, index) => (
            <motion.div
              key={index}
              whileHover={{ y: -5 }}
              className='cursor-pointer bg-gray-600 dark:bg-gray-800 flex flex-col justify-center items-center p-4 md:w-128 md:h-128 rounded-md'
            >
              ...
              <h1 className='text-gray-100 dark:text-white mt-2 text-sm'>{icon.title}</h1>
            </motion.div>
          ))}
        </div>
      </div>
